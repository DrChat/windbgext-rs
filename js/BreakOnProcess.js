...
function handleProcessCreation(processName, processCommand)
{
    ...
    var hproc = host.memory.readMemoryValues(rcx, 1, 8);
    var hthrd = host.memory.readMemoryValues(rdx, 1, 8);
    log(`[+] New process / thread handle: ${hproc} / ${hthrd}`);
    ...
    // is incapable of doing: basic pointer arithmetic.
    ...
    log(`[+] New process / thread obj: ${eprocess} / ${kthread}`);
    
    // Set process to the new created one
    execute(`.process /i ${eprocess}`);
    // execute(`.thread /r /p ${kthread}`);
    execute("g");
